...
public class MappingComponentGmlFileDropHandler implements MappingComponentDropHandler, LayerWidgetProvider {
    ...
    private static final Collection<String> EXTENSIONS = Arrays.asList(
        DocumentFeatureServiceFactory.XML_FILE_EXTENSION,
        DocumentFeatureServiceFactory.GML_FILE_EXTENSION
    );

    //~ Instance fields --------------------------------------------------------

    @Getter
    private final MappingComponentDropHandlerFileMatcher fileMatcher = new MappingComponentDropHandlerFileExtensionMatcher(
        EXTENSIONS
    );
    ...
    public int getPriority() {
        return MappingComponentDropHandlerBuiltinPriorityConstants.GML;
    }
