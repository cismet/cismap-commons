...
{ 
  protected final static Logger logger = Logger.getLogger(DefaultLayerProperties.class);
  ...
  /**
   * Creates a new DefaultLayerProperties instance and initialises the style property
   * with a preconfigured BasicStyle object. All expression types are set to undefined.
   */
  ...
  /**
   * Initialises a new DefaultLayerProperties instance from an existing
   * LayerProperties object. The properties of the LayerProperties will be
   * cloned.
   *
  ...
  {
    logger.debug("setIdExpression: '" + idExpression + "', " + type);//NOI18N
  ...
  {
    logger.debug("setPrimaryAnnotationExpression: '" + primaryAnnotationExpression + "', " + type);//NOI18N
  ...
  {
    logger.debug("setSecondaryAnnotationExpression: '" + secondaryAnnotationExpression + "', " + type);//NOI18N
...
//  }
  ...
  {
    Element element = new Element(LAYER_PROPERTIES_ELEMENT);
    element.setAttribute(ConvertableToXML.TYPE_ATTRIBUTE, this.getClass().getCanonicalName());
    
    if(this.getIdExpression() != null)
    {
      element.setAttribute("idExpression", this.getIdExpression());//NOI18N
      element.setAttribute("idExpressionType", String.valueOf(this.getIdExpressionType()));//NOI18N
    }

    element.setAttribute("idExpressionEnabled", String.valueOf(this.isIdExpressionEnabled()));//NOI18N

    if(this.getPrimaryAnnotationExpression() != null)
    {
      element.setAttribute("primaryAnnotationExpression", this.getPrimaryAnnotationExpression());//NOI18N
      element.setAttribute("primaryAnnotationExpressionType", String.valueOf(this.getPrimaryAnnotationExpressionType()));//NOI18N
    }
    
    if(this.getSecondaryAnnotationExpression() != null)
    {
      element.setAttribute("secondaryAnnotationExpression", this.getSecondaryAnnotationExpression());//NOI18N
      element.setAttribute("secondaryAnnotationExpressionType", String.valueOf(this.getSecondaryAnnotationExpressionType()));//NOI18N
    }
...
//    }
    ...
    element.setAttribute("queryType", String.valueOf(this.getQueryType()));//NOI18N
    ...
    if(this.getStyle() != null)
    {
      ...
      {
        logger.error("style element could not be created", t);//NOI18N
      }
    }
    else
    {
      logger.warn("style element could not be created (is null), setting default basic style");//NOI18N
  ...
  {
    String type = element.getAttributeValue("type");//NOI18N
    if (type == null)
    {
      throw new ClassNotFoundException("madatory attribute 'type' missing in xml element");//NOI18N
    }
    Class typeClass = Class.forName(type);
    if (!typeClass.isAssignableFrom(this.getClass()))
    {
      throw new ClassNotFoundException("the XML element type '" + type + "'does not match the layer properties class '" + this.getClass().getCanonicalName() + "'");//NOI18N
    }

    if(element.getAttribute("idExpression") != null)//NOI18N
      this.setIdExpression(element.getAttributeValue("idExpression"), element.getAttribute("idExpressionType").getIntValue());//NOI18N

    if(element.getAttribute("primaryAnnotationExpression") != null)//NOI18N
      this.setPrimaryAnnotationExpression(element.getAttributeValue("primaryAnnotationExpression"), element.getAttribute("primaryAnnotationExpressionType").getIntValue());//NOI18N

    if(element.getAttribute("idExpressionEnablesd") != null)//NOI18N
      this.setIdExpressionEnabled(element.getAttribute("idExpressionEnabled").getBooleanValue());//NOI18N

    if(element.getAttribute("secondaryAnnotationExpression") != null)//NOI18N
      this.setSecondaryAnnotationExpression(element.getAttributeValue("secondaryAnnotationExpression"), element.getAttribute("secondaryAnnotationExpressionType").getIntValue());//NOI18N
...
//      this.setQueryTemplate(element.getAttributeValue("queryTemplate"), element.getAttribute("queryType").getIntValue());
    ...
    if(element.getAttribute("queryType") != null)//NOI18N
      this.setQueryType(element.getAttribute("queryType").getIntValue());//NOI18N

    Element styleElement = element.getChild(Style.STYLE_ELEMENT);
    ...
    {
      logger.warn("initFromElement: restoring object from deprecated xml element");//NOI18N
    ...
    {
      logger.warn("no style found in XML Element, setting default random Basic Style");//NOI18N
      this.setStyle(new BasicStyle());
    }
  }
  ...
  {
    if(layerProperties.getIdExpression()!=null)this.setIdExpression(new String(layerProperties.getIdExpression()), layerProperties.getIdExpressionType());
    else this.idExpressionType = EXPRESSIONTYPE_UNDEFINED;

    if(layerProperties.getPrimaryAnnotationExpression()!=null)this.setPrimaryAnnotationExpression(new String(layerProperties.getPrimaryAnnotationExpression()), layerProperties.getPrimaryAnnotationExpressionType());
    else this.primaryAnnotationExpressionType = EXPRESSIONTYPE_UNDEFINED;
    
    if(layerProperties.getSecondaryAnnotationExpression()!=null)this.setSecondaryAnnotationExpression(new String(layerProperties.getSecondaryAnnotationExpression()), layerProperties.getSecondaryAnnotationExpressionType());
    else this.secondaryAnnotationExpressionType = EXPRESSIONTYPE_UNDEFINED;
    ...
    {
      logger.warn("unexpected CloneNotSupportedException while cloning Style object, setting default Basic Style", ex);//NOI18N
      this.setStyle(new BasicStyle());
    }
  }
