...
public abstract class AbstractFeatureFactory<FT extends FeatureServiceFeature, QT> implements FeatureFactory<FT, QT>
{
  ...
  public void setLayerProperties(LayerProperties layerProperties)
  {
    ...
    {
      logger.warn("factory supports automatic id generation, disabling id expression support in layer properties");//NOI18N
    ...
    if (this.lastCreatedfeatureVector.size() > 0)
    {
      ...
      logger.debug(this.lastCreatedfeatureVector.size() + " last created features found, applying updated expressions if applicable");//NOI18N
      ...
      {
        logger.debug("expressions did not change, re-elevation not neccessary");//NOI18N
      ...
      {
        logger.debug("re-evaluation not necessary, no supported expressions");//NOI18N
        for (FT feature : this.lastCreatedfeatureVector)
        {
          feature.setLayerProperties(this.layerProperties);
        }
      ...
      logger.debug("updating layer properties of " + this.lastCreatedfeatureVector.size() + " features took " + ((System.currentTimeMillis() - start)) + " ms");//NOI18N
    } else
    {
  ...
  {
    logger.debug("SW[" + workerThread + "]: performing re-evaluation of the expressions of " + features.size() + " selected features");//NOI18N
    long start = System.currentTimeMillis();
    int i = 0;
    for (FT feature : features)
    {
      // check if thread is canceled .........................................
      if (this.checkCancelled(workerThread, " evaluating expression"))//NOI18N
      {
        return;
      }
      // check if thread is canceled .........................................
    ...
    logger.debug("SW[" + workerThread + "]: re-evaluation of " + features.size() + " features took " + ((System.currentTimeMillis() - start)) + " ms");//NOI18N
  }
  ...
  protected void evaluateExpressions(FT feature, int index)
  {
    ...
    {
      if (DEBUG)logger.debug("evaluating idExpression '" + this.layerProperties.getIdExpression() + "' of type " + this.layerProperties.getIdExpressionType());//NOI18N
      switch (this.layerProperties.getIdExpressionType())
      {
        case LayerProperties.EXPRESSIONTYPE_PROPERTYNAME:
          if (DEBUG)
          {
            logger.debug("evaluating idExpression: EXPRESSIONTYPE_PROPERTYNAME " + LayerProperties.EXPRESSIONTYPE_PROPERTYNAME);//NOI18N
          }
          ...
          try
          {
            if (property != null)
            {
              if (DEBUG)
              {
                logger.debug("evaluating idExpression: property '" + property + "'");//NOI18N
              }
            ...
            } else
            {
              ...
              if (DEBUG)
              {
                logger.warn("evaluating idExpression: property '" + this.layerProperties.getIdExpression() + "' not found, setting id to " + ID);//NOI18N
              }
            }
          } catch (NumberFormatException nfe)
          {
            ...
            if (DEBUG)
            {
              logger.warn("evaluating idExpression: property '" + property.toString() + "' could not be converted to int, setting id to " + ID);//NOI18N
            }
          }
          break;
        ...
        case LayerProperties.EXPRESSIONTYPE_BEANSHELL:
          if (DEBUG)
          {
            logger.debug("evaluating idExpression: EXPRESSIONTYPE_BEANSHELL " + LayerProperties.EXPRESSIONTYPE_BEANSHELL);//NOI18N
          }
        ...
        case LayerProperties.EXPRESSIONTYPE_GROOVY:
          if (DEBUG)
          {
            logger.debug("evaluating idExpression: EXPRESSIONTYPE_GROOVY " + LayerProperties.EXPRESSIONTYPE_GROOVY);//NOI18N
          }
    ...
    if (DEBUG)
    {
      logger.debug("evaluating PrimaryAnnotationExpression '" + this.layerProperties.getPrimaryAnnotationExpression() + "' of type " + this.layerProperties.getPrimaryAnnotationExpressionType());//NOI18N
    }
    switch (this.layerProperties.getPrimaryAnnotationExpressionType())
    {
      ...
      case LayerProperties.EXPRESSIONTYPE_PROPERTYNAME:
        ...
        if (DEBUG)
        {
          logger.debug("evaluating PrimaryAnnotationExpression: setting PrimaryAnnotationExpression '" + property + "'");//NOI18N
        }
        if (property != null)
        {
    ...
    if (DEBUG)
    {
      logger.debug("evaluating SecondaryAnnotationExpression '" + this.layerProperties.getSecondaryAnnotationExpression() + "' of type " + this.layerProperties.getSecondaryAnnotationExpressionType());//NOI18N
    }
    switch (this.layerProperties.getSecondaryAnnotationExpressionType())
    {
      ...
      case LayerProperties.EXPRESSIONTYPE_PROPERTYNAME:
        ...
        if (DEBUG)
        {
          logger.debug("evaluating PrimaryAnnotationExpression: setting SecondaryAnnotationExpression '" + property + "'");//NOI18N
        }
        if (property != null)
        {
  ...
  protected String evaluateGroovyExpressions(FT feature, String expression)
  {
    ...
    {
      if(DEBUG)logger.debug("performing lazy first time initialisation of GroovyShell");//NOI18N
    ...
    try
    {
      ...
      for (Object key : feature.getProperties().keySet())
      {
        ...
        groovyShell.setVariable(key.toString().replaceAll(":", "_"), property);//NOI18N
      }

      expression = expression.replaceAll(":", "_");//NOI18N
      return groovyShell.evaluate(expression).toString();
    } catch (Throwable t)
    {
      logger.error("could not evaluate groovy expression '" + expression + "'", t);//NOI18N
      return null;
    }
  }
  ...
  {
    throw new UnsupportedOperationException("BeanShell not supported");//NOI18N
  }
  ...
  protected boolean checkCancelled(SwingWorker workerThread, String message)
  {
    ...
    {
      logger.warn("FRW[" + workerThread + "]: operation is canceled after " + message);//NOI18N//NOI18N
