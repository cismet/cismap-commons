...
public abstract class DegreeFeatureFactory<FT extends FeatureServiceFeature, QT> extends AbstractFeatureFactory<FT, QT>
{
  ...
  protected Vector<FT> processFeatureCollection(SwingWorker workerThread, Feature[] featureCollection, boolean evaluateExpressions) throws Exception
  {
    if (DEBUG)
    {
      logger.debug("SW[" + workerThread + "]: converting " + featureCollection.length + " degree features to FeatureServiceFeatures");//NOI18N
    }
    ...
    {
      // check if canceled .......................................................
      if (this.checkCancelled(workerThread, "converting degree features (" + i + ")"))//NOI18N
      {
    ...
    logger.info("SW[" + workerThread + "]: converting " + featureCollection.length + " degree features took " + ((System.currentTimeMillis() - start)) + " ms");//NOI18N
    return featureVector;
  }
  ...
  protected class ParsingProgressListener implements FeatureProgressListener
  {
    ...
    {
      if(DEBUG)logger.debug("real feature parsing progress: " + progress);//NOI18N
      ...
      if (workerThread != null && newProgress > currentProgress)
      {
        ...
        if (DEBUG)
        {
          logger.debug("SW[" + workerThread + "]: passing progress: " + currentProgress + "%");//NOI18N
        }
