...
public class WFSFeatureFactory extends DegreeFeatureFactory<WFSFeature, String>
{
  ...
  public static final String CISMAP_BOUNDING_BOX_AS_GML_PLACEHOLDER = "<cismapBoundingBoxAsGmlPlaceholder />";//NOI18N
  ...
  {
    logger.info("initialising WFSFeatureFactory with hostname: '" + hostname + "'");//NOI18N
  ...
  public Vector<WFSFeature> createFeatures(String query, BoundingBox boundingBox, SwingWorker workerThread) throws TooManyFeaturesException, Exception
  {
    ...
    // check if canceled .......................................................
    if (this.checkCancelled(workerThread, "createFeatures()"))//NOI18N
    {
      return null;
    }
    // check if canceled .......................................................
    ...
    // check if canceled .......................................................
    if (this.checkCancelled(workerThread, "creating post string"))//NOI18N
    {
      return null;
    }
    // check if canceled .......................................................
    ...
    if (DEBUG)
    {
      logger.debug("FRW[" + workerThread + "]: Feature post request: <br><pre>" + StaticHtmlTools.stringToHTMLString(postString) + "</pre>");//NOI18N
    }

    // check if canceled .......................................................
    if (this.checkCancelled(workerThread, "creating http request"))//NOI18N
    {
    ...
    if (proxySet != null && proxySet.equals("true"))//NOI18N
    {
      if (DEBUG)
      {
        logger.debug("proxyIs Set");//NOI18N
      ...
      {
        logger.warn("Problem while setting proxy: " + e.getMessage(), e);//NOI18N
      }
    }
    client.executeMethod(httppost);

    // check if canceled .......................................................
    if (this.checkCancelled(workerThread, "executing http request"))//NOI18N
    {
      this.cleanup(null, httppost);
      return null;
    }
    ...
    logger.info("FRW[" + workerThread + "]: WFS request took " + ((System.currentTimeMillis() - start)) + " ms");//NOI18N
    int code = httppost.getStatusCode();

    if (httppost.getStatusCode() != HttpStatus.SC_OK)
    {
      logger.error("FRW[" + workerThread + "]: connection failed with HTTP Status Code '" + httppost.getStatusCode() + "'");//NOI18N
      httppost.releaseConnection();
      Exception ex = new Exception("FRW[" + workerThread + "]: Status code: " + code + " unknown transaction cancelled");//NOI18N
      ex.initCause(new Throwable("FRW[" + workerThread + "]: Status code: " + code + " unknown transaction cancelled"));//NOI18N
      throw ex;
    }
    ...
    // check if canceled .......................................................
    if (this.checkCancelled(workerThread, "creating InputStreamReader"))//NOI18N
    {
    ...
    try
    {
      ...
      // check if canceled .......................................................
      if (this.checkCancelled(workerThread, "creating GMLFeatureCollectionDocument"))//NOI18N
      {
        this.cleanup(reader, httppost);
        return null;
      }
      ...
      featureCollectionDocument.load(reader, "http://dummyID");//NOI18N

      // check if canceled .......................................................
      if (this.checkCancelled(workerThread, "loading features"))//NOI18N
      {
        this.cleanup(reader, httppost);
        return null;
      }
      ...
      {
        logger.warn("FRW[" + workerThread + "]: no features found before parsing");//NOI18N
      ...
      if (DEBUG)
      {
        logger.debug("FRW[" + workerThread + "]: parsing " + featureCollectionDocument.getFeatureCount() + " features");//NOI18N
      }

      featureCollection = featureCollectionDocument.parse();

      // check if canceled .......................................................
      if (this.checkCancelled(workerThread, "parsing features"))//NOI18N
      {
        this.cleanup(reader, httppost);
        return null;
      }
      ...
      logger.info("FRW[" + workerThread + "]: parsing " + featureCollection.size() + " features took " + ((System.currentTimeMillis() - start)) + " ms");//NOI18N
      this.cleanup(reader, httppost);

      if (featureCollection.size() > this.getMaxFeatureCount())
      {
        throw new TooManyFeaturesException("FRW[" + workerThread + "]: feature in feature document " + featureCollection.size() + " exceeds max feature count " + this.getMaxFeatureCount());//NOI18N
      } else if (featureCollection.size() == 0)
      {
        logger.warn("FRW[" + workerThread + "]: no features found after parsing");//NOI18N
        return null;
      }
    ...
    {
      logger.error("FRW[" + workerThread + "]: error parsing features: " + t.getMessage(), t);//NOI18N
      throw t;
    }
  }
  ...
  {
    throw new UnsupportedOperationException("LIW[" + workerThread + "]: WFSFeatureFactory does not support Attributes");//NOI18N
  }
