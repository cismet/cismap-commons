...
public class GMLFeatureFactory extends DegreeFeatureFactory<DefaultFeatureServiceFeature, String> implements CachingFeatureFactory<DefaultFeatureServiceFeature, String>
{
  ...
  public GMLFeatureFactory(LayerProperties layerProperties, URI documentURL, int maxCachedFeatureCount, SwingWorker workerThread) throws Exception
  {
    ...
    {
      logger.error("SW[" + workerThread + "]: error parsing gml file", ex);//NOI18N
  ...
  {
    logger.info("SW[" + workerThread + "]: initialising GMLFeatureFactory with document: '" + documentURI + "'");//NOI18N
    ...
    this.gmlDocument.load(this.documentReader, "http://dummyID");//NOI18N
    this.gmlDocument.getFeatureCount();

    //check if thread is canceled .........................................
    if (this.checkCancelled(workerThread, " initialising gml document "))//NOI18N
    {
      this.cleanup();
      return;
    }
    ...
    if (DEBUG)
    {
      logger.debug("SW[" + workerThread + "]: " + max + " features found in gml file");//NOI18N
    }
    if (max > this.maxCachedFeatureCount)
    {
      logger.error("SW[" + workerThread + "]: number of features in gml file (" + max + ") exceeds maximum of supported features (" + this.maxCachedFeatureCount + ")");//NOI18N
      max = this.maxCachedFeatureCount;
    }
    if (max == 0)
    {
      logger.error("SW[" + workerThread + "]: no features found in gml file");//NOI18N
      throw new Exception("no features found in gml file '" + this.documentURI + "'");//NOI18N
    }
    ...
    if(DEBUG) logger.debug("SW[" + workerThread + "]: " + featureCollection.size() + " features parsed");//NOI18N
    this.cleanup();

    //check if thread is canceled .........................................
    if (this.checkCancelled(workerThread, " parsing gml document "))//NOI18N
    {
      return;
    }
    // check if thread is canceled .........................................
    ...
    {
      logger.error("could not create feature service attributes, no valid gml fetures found");//NOI18N
    }

    //check if thread is canceled .........................................
    if (this.checkCancelled(workerThread, " creating feature service attributes"))//NOI18N
    {
      return;
    }
    // check if thread is canceled .........................................
    ...
    //check if thread is canceled .........................................
    if (this.checkCancelled(workerThread, " processing parsed features"))//NOI18N
    {
      return;
    }
    // check if thread is canceled .........................................

    logger.info("parsing, converting and initialising " + max + " gml features took " + ((System.currentTimeMillis() - start)) + " ms");//NOI18N
  }
  ...
  public synchronized Vector<DefaultFeatureServiceFeature> createFeatures(String query, BoundingBox boundingBox, SwingWorker workerThread) throws TooManyFeaturesException, Exception
  {
    if (!this.initialised)
    {
      logger.warn("SW[" + workerThread + "]: Factory not correclty initialised, parsing gml file");//NOI18N
      this.parseGMLFile(workerThread);
      this.initialised = true;
  ...
  public synchronized Vector<FeatureServiceAttribute> createAttributes(SwingWorker workerThread) throws TooManyFeaturesException, Exception
  {
    ...
    {
      logger.warn("SW[" + workerThread + "]: Factory not correctopy initialised, parsing gml file");//NOI18N
      this.parseGMLFile(workerThread);
    }

    if (this.featureServiceAttributes == null || this.featureServiceAttributes.size() == 0)
    {
      logger.error("SW[" + workerThread + "]: no attributes could be found in gml file");//NOI18N
      throw new Exception("no attributes could be found in gml file '" + this.documentURI + "'");//NOI18N
    }
