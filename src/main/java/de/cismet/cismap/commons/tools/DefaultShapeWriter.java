 ...
 */
@org.openide.util.lookup.ServiceProvider(position = 10, service = ShapeWriter.class)
public class DefaultShapeWriter implements ShapeWriter {
    ...
    public void writeShape(
        ...
        final FeatureCollection fc = new SimpleFeatureCollection(getId(), features, aliasAttributeList);
    ...
    public void writeDbf(
        ...
        if (fileToSaveTo.getAbsolutePath().toLowerCase().endsWith(".dbf")) {
            final String fileNameWithoutExt = fileToSaveTo
                .getAbsolutePath()
