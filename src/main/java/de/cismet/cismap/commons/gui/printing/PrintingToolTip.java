...
public class PrintingToolTip extends PNode {
    ...
    public PrintingToolTip(final Color backgroundColor) {
        final PImage image = new PImage(
            new javax.swing.ImageIcon(getClass().getResource("/de/cismet/cismap/commons/gui/res/frameprint64.png"))
                .getImage()
        ); // NOI18N
        image.setOffset(10, 10);
        final PText t1 = new PText(
            org.openide.util.NbBundle.getMessage(PrintingToolTip.class, "PrintingToolTip.PrintingToolTip(Color).t1")
        ); // NOI18N
        ...
        final PText t2 = new PText(
            org.openide.util.NbBundle.getMessage(PrintingToolTip.class, "PrintingToolTip.PrintingToolTip(Color).t2")
        ); // NOI18N
        final PText t3 = new PText(
            org.openide.util.NbBundle.getMessage(PrintingToolTip.class, "PrintingToolTip.PrintingToolTip(Color).t3")
        ); // NOI18N
        final PText t4 = new PText(
            org.openide.util.NbBundle.getMessage(PrintingToolTip.class, "PrintingToolTip.PrintingToolTip(Color).t4")
        ); // NOI18N

        final double textHeight = t1.getHeight() + 5 + t2.getHeight() + 5 + t3.getHeight() + 5 + t4.getHeight();
        final double textWidth = Math.max(
            Math.max(t1.getWidth(), t2.getWidth()),
        ...
        final PPath background = new PPath(
            new RoundRectangle2D.Double(0, 0, 10 + image.getWidth() + textWidth + 10, 5 + backgroundHeight + 5, 10, 10)
        );
