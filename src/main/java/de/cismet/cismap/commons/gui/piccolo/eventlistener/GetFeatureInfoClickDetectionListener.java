...
public class GetFeatureInfoClickDetectionListener extends PBasicInputEventHandler implements HoldListener {
     ...
     * @throws  IllegalStateException  DOCUMENT ME!
     */
    private void showCustomFeatureInfo(
        final Collection<SignaturedFeature> c,
        final MultipleFeatureInfoRequestsDisplay display
    ) {
        final MappingComponent mc = CismapBroker.getInstance().getMappingComponent();

        // TODO if !display.isOnHold paint the standard icon...
        if (/*!display.isOnHold()*/!display.isDisplayVisible() || ((c == null) || c.isEmpty())) {
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                    "MultipleFeauteInfoRequestsDisplay is not on hold or holdFeautureCollection is null or empty."
                ); // NOI18N
            }
            return;
        } else {
            ...
            try {
                final InputStream in = getClass()
                    .getResourceAsStream("/de/cismet/cismap/commons/gui/res/featureInfoIcon.properties"); // NOI18N
                if (in != null) {
                    iconProps.load(in);
                    in.close();
                } else {
                    LOG.warn(
                        "Could not laod featureInfoIcon.properties file. Default values for overlay area are used"
                    ); // NOI18N
                }
            } catch (IOException ex) {
                LOG.error(
            ...
            if (
                ...
                LOG.warn(
                    "featureInfoIcon.properties file does not contain all needed keys. Default values for overlay area are used"
                ); // NOI18N
            } else {
                try {
            ...
            for (final SignaturedFeature f : c) {
                ...
                if (f.getOverlayIcon() == null) {
                    ...
                    Log.warn(
                        "OverlayIcon for MultipleFeautreInfoRequestDisplay is null, default FeatureInfoIcon is used"
                    ); // NOI18N
                } else {
                    if ((nr == (c.size() - 1)) && (lastFeatureInfoIcon != null)) {
                        // default
                        g2d =
                            (Graphics2D) lastFeatureInfoIcon
                                .getSubimage(lastFeatureInfoIcon.getWidth() - 16, 0, 16, 16)
                                .getGraphics();
                        try {
