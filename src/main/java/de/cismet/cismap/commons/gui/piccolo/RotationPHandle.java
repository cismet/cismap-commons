...
public class RotationPHandle extends PHandle {
    ...
    public void startHandleDrag(Point2D aLocalPoint, PInputEvent aEvent) {
        if (pfeature.getViewer().isFeatureDebugging()) {
            log.debug("startHandleDrag");//NOI18N
        }
        rotation = 0.0d;
    ...
    public void endHandleDrag(java.awt.geom.Point2D aLocalPoint, PInputEvent aEvent) {
        try {
            if (pfeature.getViewer().isFeatureDebugging()) {
                log.debug("endHandleDrag");//NOI18N
            }
            ...
            log.debug("after fireFeaturesChanged");//NOI18N
            ...
            log.debug("After Overlap-Check");//NOI18N
            if (overlap) {
                log.debug("overlaps");//NOI18N
                if (pfeature.getViewer().isFeatureDebugging()) {
                    log.debug("Overlaps after rotation of the PFeatures");//NOI18N
                }
                ...
                if (pfeature.getViewer().isFeatureDebugging()) {
                    log.debug("perform rotation: " + (answer == JOptionPane.YES_OPTION ? "Yes" : "No"));//NOI18N
                }
                ...
                log.debug("doesn't overlap");//NOI18N
                if (rotation != 0.0d) {
                    log.debug("arr=" + selArr);//NOI18N
                    log.debug("mid=" + mid);//NOI18N
