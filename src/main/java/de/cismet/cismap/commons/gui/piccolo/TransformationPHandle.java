    ...
    public void dragHandle(PDimension aLocalDimension, PInputEvent pInputEvent) {
        if (!pfeature.getViewer().getInteractionMode().equals(MappingComponent.SPLIT_POLYGON)) {
            ...
            if (moveListener != null) {
                ...
                log.warn("Movelistener to adjust the mouse pointer not found.");//NOI18N
            }
...
//                        }
                ...
                if (pfeature.getViewer().isFeatureDebugging()) {
                    log.debug("Width,Height:" + (float) aLocalDimension.getWidth() + "," + (float) aLocalDimension.getHeight());//NOI18N
                }
                if (pfeature.getViewer().isFeatureDebugging()) {
                    log.debug("Width,Height:" + aLocalDimension.getWidth() + "," + aLocalDimension.getHeight());//NOI18N
                }
                if (pfeature.getViewer().isFeatureDebugging()) {
                    log.debug("old.value:" + pfeature.getXp()[positionInArray] + "," + pfeature.getYp()[positionInArray]);//NOI18N
                }
                if (pfeature.getViewer().isFeatureDebugging()) {
                    log.debug("new.value:" + newX + "," + newY);//NOI18N
                }
                if (pfeature.getViewer().isFeatureDebugging()) {
                    log.debug("calculation:" + (float) ((double) pfeature.getXp()[positionInArray] * 10000 + (double) aLocalDimension.getWidth() * 10000) / 10000);//NOI18N
                }
                        ...
                        if (gluePFeature.getFeature().isEditable()) {
                            ...
                            if (coordinates != null) {
                                for (Object o : coordinates) {
                                    ...
                                    if (pfeature.getViewer().isFeatureDebugging()) {
                                        log.debug("moveCoordinateToNewPiccoloPosition " + gluePFeature);//NOI18N
                                    }
                                }
                            }
                        }
...
//                        }
                ...
                leftInfo.setText("" + StaticDecimalTools.round(leftDistance));//NOI18N
                rightInfo.setText("" + StaticDecimalTools.round(rightDistance));//NOI18N
        ...
        if (
            ...
            if (!pfeature.getViewer().getInteractionMode().equals(MappingComponent.SPLIT_POLYGON)) {
                ...
                if (pfeature.getViewer().isInGlueIdenticalPointsMode()) {
                    ...
                    log.info("checkforGlueCoords() called and " + glueCoordinates.keySet().size() + " found");//NOI18N
                }
    ...
    public void endHandleDrag(java.awt.geom.Point2D aLocalPoint, PInputEvent aEvent) {
        if (!pfeature.getViewer().getInteractionMode().equals(MappingComponent.SPLIT_POLYGON)) {
            ...
            if ((pfeature.getViewer().getHandleInteractionMode().equals(MappingComponent.MOVE_HANDLE)) &&
                ...
                if (pfeature.getViewer().isFeatureDebugging()) {
                    log.debug("create new MoveAction");//NOI18N
                }
...
//                                                gluePFeature.syncGeometry();
                                        if (pfeature.getViewer().isFeatureDebugging()) {
                                            log.debug("PFeature synced:" + gluePFeature);//NOI18N
                                        }
                                    }
                                }
                            }
    ...
    public void handleClicked(edu.umd.cs.piccolo.event.PInputEvent pInputEvent) {
        if (pfeature.getViewer().isFeatureDebugging()) {
            log.debug("Handle clicked");//NOI18N
        }
        ...
        if (pfeature.getViewer().isFeatureDebugging()) {
            log.debug("End of handleClicked() getFeature().getGeometry().getCoordinates().length:" + pfeature.getFeature().getGeometry().getCoordinates().length);//NOI18N
        }
    ...
    public void removeHandle() {
        ...
        if (pfeature.getXp().length > 4 && pfeature.getFeature().getGeometry() instanceof Polygon ||
            ...
            log.debug("---vorher:" + pfeature.getCoordArr().length);//NOI18N
            pfeature.setCoordArr(pfeature.removeCoordinateFromOutside(positionInArray, pfeature.getCoordArr()));
            pfeature.syncGeometry();
            log.debug("---nachher:" + pfeature.getCoordArr().length);//NOI18N
    ...
    public void duplicateHandle() {
        log.info("duplicate Handle : handle count before:" + pfeature.getCoordArr().length);//NOI18N
        ...
        pfeature.syncGeometry();
        log.info("duplicate Handle : handle count after:" + pfeature.getCoordArr().length);//NOI18N
        pfeature.setPathToPolyline(pfeature.getXp(), pfeature.getYp());
        pfeature.getViewer().showHandles(false);
        Vector v = new Vector();
        v.add(pfeature.getFeature());
