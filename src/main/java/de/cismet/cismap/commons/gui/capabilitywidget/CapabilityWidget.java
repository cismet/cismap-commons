...
import de.cismet.cismap.commons.raster.tms.TMSCapabilitiesTreeCellRenderer;
import de.cismet.cismap.commons.raster.tms.TMSCapabilitiesTreeModel;
import de.cismet.cismap.commons.raster.tms.tmscapability.TMSCapabilities;
import de.cismet.cismap.commons.raster.tms.tmscapability.TMSCapabilitiesFactory;
...
public class CapabilityWidget extends JPanel implements DropTargetListener, ChangeListener, ActionListener, Configurable, MapBoundsListener {
    ...
    private HashMap<Component, TMSCapabilities> tmsCapabilities = new HashMap<Component, TMSCapabilities>();
    private HashMap<Component, JTree> tmsCapabilitiesTrees = new HashMap<Component, JTree>();
    ...
    private void processUrl(final String link, final String subparent, final boolean interactive) {
        ...
        EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                if (link.toLowerCase().contains("service=wfs")) {//NOI18N
                ...
                } else if(link.toLowerCase().contains("service=tms")){//NOI18N
                    addTMSCapabilitiesTree(link, load, interactive);
                    ...
                    Object[] alternatives = {"OGC-Web Mapping Service", "OGC-Web Feature Service", "OGC-Web Security Service", "WMS Tiling Client"};//NOI18N
                    ...
                    } else if (selectedValue == alternatives[3]) {
                        addTMSCapabilitiesTree(link, load, interactive);
...
//                    }
    ...
    private void addTMSCapabilitiesTree(final String link, final JComponent comp, final boolean interactive){
        log.debug("Add TMSCapabilitiesTree");//NOI18N
...
//                    final WMSCapabilities cap = capFact.createCapabilities(HttpAuthentication.getInputStreamReaderFromURL(CapabilityWidget.this, getCapURL));
                    final WMSCapabilities cap = capFact.createCapabilities(  new InputStreamReader(WebAccessManager.getInstance().doRequest(getCapURL)) );
                    final TMSCapabilities tms = TMSCapabilitiesFactory.parseTMSCapabilities(cap);
//                    broker.addHttpCredentialProviderCapabilities(cap, broker.getHttpCredentialProviderURL(getCapURL));
//                    if (HttpAuthentication.isServerSecuredByPassword(cap)) {
//                        broker.addProperty(getCapURL.toString(), cap.getCapability().getLayer().getTitle());
//                    }
                    trvCap.setWmsCapabilities(cap);
                    final TMSCapabilitiesTreeModel tm = new TMSCapabilitiesTreeModel(tms);
                    ...
                    EventQueue.invokeLater(new Runnable() {
                        ...
                        public void run() {
                            trvCap.setModel(tm);
                            trvCap.setBorder(new EmptyBorder(1, 1, 1, 1));
                            trvCap.setCellRenderer(new TMSCapabilitiesTreeCellRenderer());
                            final JScrollPane sPane = new JScrollPane();
                            sPane.setViewportView(trvCap);
                            sPane.setBorder(new EmptyBorder(1, 1, 1, 1));
                            StaticSwingTools.setNiftyScrollBars(sPane);
                            ...
                            tmsCapabilities.put(sPane, tms);
                            log.debug("tmsCapabilities.length: " + tmsCapabilities.size());//NOI18N
                            tmsCapabilitiesTrees.put(sPane, trvCap);
                            stateChanged(null);
                    ...
                    if (interactive) {
                        JOptionPane.showMessageDialog(thisWidget,
                                org.openide.util.NbBundle.getMessage(CapabilityWidget.class, "CapabilityWidget.addTMSCapabilitiesTree.JOptionPane.message", new Object[] {message}),//NOI18N
                                org.openide.util.NbBundle.getMessage(CapabilityWidget.class, "CapabilityWidget.addTMSCapabilitiesTree.JOptionPane.title"),//NOI18N
                                JOptionPane.ERROR_MESSAGE);
                    }
                    //TODO: Error \u00FCber die Statuszeile bekanntgeben
                    log.error("Error while loading server capabilities: " + message, e);//NOI18N
...
// TODO Hier WFS, ESRI, Google, ...
    ...
    class DragTree extends JTree implements DragGestureListener, DragSourceListener {
        ...
        public void dragGestureRecognized(DragGestureEvent e) {
            ...
            } else if(this.getModel() instanceof TMSCapabilitiesTreeModel){
                if (getSelectionModel().getSelectionPath().getLastPathComponent() instanceof TileSet){
                    final TileSet t = (TileSet)getSelectionModel().getSelectionPath().getLastPathComponent();
                    if (log.isDebugEnabled()) {
                        log.debug("TileSet found: " + t.getLayer());//NOI18N
                    }
                    trans = new DefaultTransferable(t);
                }
