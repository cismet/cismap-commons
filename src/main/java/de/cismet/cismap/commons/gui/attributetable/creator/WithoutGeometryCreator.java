...
public class WithoutGeometryCreator extends AbstractFeatureCreator {
    ...
    public void createFeature(final MappingComponent mc, final FeatureServiceFeature feature) {
        ...
        if (feature instanceof DefaultFeatureServiceFeature) {
            try {
                fillFeatureWithDefaultValues((DefaultFeatureServiceFeature) feature, properties);
                ((DefaultFeatureServiceFeature) feature).saveChanges();
                fillFeatureWithDefaultValuesAfterSave((DefaultFeatureServiceFeature) feature, properties);

                for (final FeatureCreatedListener featureCreatedListener : this.listener) {
                    featureCreatedListener.featureCreated(
                        new FeatureCreatedEvent(WithoutGeometryCreator.this, feature)
                    );
                }
            } catch (Exception e) {
