...
import de.cismet.cismap.commons.raster.tms.simple.TileLayer;
    ...
    public Element getConfiguration() throws NoWriteError {
        ...
        while (it.hasNext()) {
            ...
            if (service instanceof SimpleFeatureSupportingRasterLayer) {
                ...
                allLayerConf.addContent(layerConf);
                counter++;
            } else if (service instanceof TileLayer){
                log.debug("TileLayer found. Will be added to configuration.xml");//NOI18N
                Element layerConf = ((TileLayer)service).getElement();
                allLayerConf.addContent(layerConf);
                counter++;
    ...
    private void createLayers(final Element conf, final HashMap<String, WMSCapabilities> capabilities) {
        ...
        for (final Element element : orderedLayers) {
            ...
            try {
                ...
                // <editor-fold defaultstate="collapsed" desc="TMSLayer">
                else if (element.getName().equals("TMSLayer")) {//NOI18N
                    log.debug("TMSLayer found. Try to add the layer to the ActiveLayerModel.");//NOI18N
                    String host = element.getChildText("Host");//NOI18N
                    String version = element.getChildText("Version");//NOI18N
                    String srs = element.getChildText("SRS");//NOI18N
                    String layer = element.getChildText("Layers");//NOI18N
                    String format = element.getChildText("Format");//NOI18N
                    String styles = element.getChildText("Styles");//NOI18N
                    int width = Integer.parseInt(element.getChildText("Width"));//NOI18N
                    int height = Integer.parseInt(element.getChildText("Height"));//NOI18N
                    Element bbElement = element.getChild("BoundingBox");//NOI18N
                    BoundingBox newBB = new BoundingBox(
                            Double.parseDouble(bbElement.getAttribute("minx").getValue()),//NOI18N
                            Double.parseDouble(bbElement.getAttribute("miny").getValue()),//NOI18N
                            Double.parseDouble(bbElement.getAttribute("maxx").getValue()),//NOI18N
                            Double.parseDouble(bbElement.getAttribute("maxy").getValue()));//NOI18N
                    String res = element.getChildText("Resolutions");//NOI18N
                    String[] resArray = res.split(",");//NOI18N
                    Double[] resolutions = new Double[resArray.length];
                    for (int i = 0; i < resArray.length; i++){
                        resolutions[i] = Double.parseDouble(resArray[i]);
                    }
//                    log.fatal("SRS: "+srs+ " und layer: "+layer +" format: "+format+
//                            " width: "+width+ " height: "+height+ " resolutions: "+
//                            Arrays.toString(resolutions));
                    TileSet newTileSet = new TileSet(version,host,srs,newBB,
                            resolutions, width, height,format,layer, styles);
                    TileLayer t = new TileLayer(CismapBroker.getInstance().getMappingComponent(), newTileSet);
                    log.debug("TileSet and TileLayer created!");//NOI18N
                    addLayer(t);
                    log.debug("TileLayer added to the ActiveLayerModel.");//NOI18N
                }
